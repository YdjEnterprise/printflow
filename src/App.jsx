...
// ── Constants ──────────────────────────────────────────────────────
const DEFAULT_ORDER_TYPES = ["Vinyl Print","LED Board","Flex Banner","Sticker","Standee","Hoarding","Neon Sign","Canvas Print","Metal Sign","Glass Print","ACP Board","Backlit Board"];
const DEFAULT_STAGES = ["Order Received","Design In Progress","Awaiting Approval","In Fabrication","Printing","Finishing","Ready for Delivery","Pending Billing","Bill Posted/Given","Delivered"];
...
function getPaymentAlerts(jobs,clients) {
  return jobs.filter(j=>{
    ...
    return ["Delivered","Pending Billing","Bill Posted/Given","Ready for Delivery"].includes(j.stage);
  }).map(j=>{
    const ds=daysSince(j.deliveredAt||j.createdAt)||0;
    const level=ds>10?"red":ds>5?"orange":ds>2?"yellow":"new";
    return {...j,daysAfterDelivery:ds,payLevel:level};
  }).sort((a,b)=>b.daysAfterDelivery-a.daysAfterDelivery);
}
...
function App(){
  ...
  function deleteJob(orderId){
    setJobs(prev=>prev.filter(j=>j.orderId!==orderId));
    setPrintJobs(prev=>prev.filter(p=>p.orderId!==orderId));
    setFabJobs(prev=>prev.filter(f=>f.orderId!==orderId));
  ...
  useEffect(()=>{
    loadAll().then(async (data)=>{
      ...
      try{
        ...
        const savedGifts=await loadSettings("loyalty_gifts");
        if(savedGifts&&savedGifts.length>0)setLoyaltyGifts(savedGifts);
        const savedTypes=await loadSettings("order_types");
        if(savedTypes&&savedTypes.length>0)setOrderTypes(savedTypes);
        const savedStages=await loadSettings("stages");
        if(savedStages&&savedStages.length>0)setStages(savedStages);
      }catch(e){}
      setLoading(false);
  ...
  // Auto-sync print/fab sub-job stages when main job stage changes
  useEffect(()=>{
    // Update print sub-jobs
    setPrintJobs(prev=>{
      let changed=false;
      const result=prev.map(pj=>{
        const parent=jobs.find(jj=>jj.orderId===pj.orderId);
        if(!parent) return pj;
        let ns=pj.stage;
        if(parent.stage==="Printing"&&pj.stage!=="Printing"&&pj.stage!=="Done")ns="Printing";
        else if(parent.stage==="Finishing"&&pj.stage!=="Done")ns="Lamination/Finishing";
        else if((parent.stage==="Delivered"||parent.stage==="Ready for Delivery"||parent.stage==="Pending Billing"||parent.stage==="Bill Posted/Given")&&pj.stage!=="Done")ns="Done";
        if(ns!==pj.stage){changed=true; return {...pj,stage:ns};}
        return pj;
      });
    ...
    setFabJobs(prev=>{
      let changed=false;
      const result=prev.map(fj=>{
        const parent=jobs.find(jj=>jj.orderId===fj.orderId);
        if(!parent) return fj;
        let ns=fj.stage;
        if(parent.stage==="In Fabrication"&&fj.stage!=="In Fabrication"&&fj.stage!=="Ready"&&fj.stage!=="Dispatched")ns="In Fabrication";
        else if((parent.stage==="Delivered"||parent.stage==="Ready for Delivery")&&fj.stage!=="Dispatched")ns="Dispatched";
        if(ns!==fj.stage){changed=true; return {...fj,stage:ns};}
        return fj;
      });
      return changed?result:prev;
    });
  ...
  const sharedProps={jobs,setJobs,clients,setClients,fabJobs,setFabJobs,printJobs,setPrintJobs,showToast,setPage,billingAlerts,paymentAlerts,setBillAlert,loyaltyGifts,setLoyaltyGifts,orderTypes,setOrderTypes,stages,setStages,deleteJob};
  ...
  return(
    <div style={S.root}>
      ...
      <div style={S.main}>
        ...
        {page==="receivables" && <Receivables   {...sharedProps}/>}
        {page==="completed"   && <CompletedJobs {...sharedProps}/>}
...
function Sidebar({page,setPage,resetData,confirmReset,billingAlerts,paymentAlerts,fabJobs,printJobs}){
  ...
  const nav=[
    ...
    {id:"receivables",icon:"💳",label:"Receivables",  badge:paymentAlerts.filter(x=>x.payLevel==="red").length},
...
function TopBar({page}){
  const titles={dashboard:"Dashboard",add:"Add New Job",pending:"Pending Jobs",print:"Print Department",fabrication:"Fabrication Department",clients:"Client Database",billing:"Billing",receivables:"Pending Receivables",completed:"Completed Jobs",reports:"Reports & Analytics",settings:"Settings"};
  return(
    <div style={S.topBar}>
...
// ── Dashboard ──────────────────────────────────────────────────────
function Dashboard({jobs,setJobs,clients,showToast,setPage,billingAlerts,paymentAlerts,setBillAlert,fabJobs,printJobs,stages,deleteJob}){
        ...
        <div style={{...S.card,borderLeft:"4px solid #ff3b3b",background:"#1a0808",marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:6}}>
              <button onClick={()=>setPage("receivables")} style={{background:"#ff3b3b22",color:"#ff3b3b",border:"1px solid #ff3b3b44",borderRadius:6,padding:"3px 10px",cursor:"pointer",fontSize:10,fontWeight:700}}>View →</button>
      ...
      <div style={S.card}>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={S.table}>
            ...
            <tbody>{[...jobs].sort((a,b)=>URGENCY.indexOf(b.urgency)-URGENCY.indexOf(a.urgency)).map(j=><JobRow key={j.orderId} job={j} jobs={jobs} setJobs={setJobs} showToast={showToast} stages={stages} deleteJob={deleteJob}/>)}</tbody>
          </table>
        </div>
      </div>
    </div>
...
function JobRow({job,jobs,setJobs,showToast,stages,deleteJob}){
    ...
    <tr style={S.tr}>
      ...
      <td style={S.td}>
        <div style={{display:"flex",gap:3,alignItems:"center"}}>
          ...
          {!confirmDel?<button onClick={()=>setConfirmDel(true)} style={{background:"none",border:"none",color:"#ff3b3b55",cursor:"pointer",fontSize:11,padding:"1px 4px"}}>🗑</button>
          :<button onClick={()=>{deleteJob(job.orderId);setConfirmDel(false);}} style={{background:"#ff3b3b22",border:"1px solid #ff3b3b44",color:"#ff3b3b",cursor:"pointer",fontSize:9,padding:"1px 6px",borderRadius:4,fontWeight:700}}>Delete?</button>}
        </div>
      </td>
    </tr>
  );
...
// ── Add Job ────────────────────────────────────────────────────────
function AddJob({jobs,setJobs,clients,setClients,fabJobs,setFabJobs,printJobs,setPrintJobs,showToast,setPage,orderTypes,stages}){
  ...
  const mkPrint=()=>({_id:Date.now()+Math.random(),type:"Digital Flex",material:"Star Flex",width:"",height:"",qty:"1",rate:"",resolution:"720dpi",notes:""});
  const mkFab=()=>({_id:Date.now()+Math.random(),type:"MS Frame",pipeSize:"1\"",pipeThick:"2mm",woodType:"Plywood 18mm",acpThick:"4mm",width:"",height:"",qty:"1",rate:"",notes:"",stage:"Pending"});
  const[form,setForm]=useState(blank);
  const[printSpecs,setPrintSpecs]=useState([mkPrint()]);
  const[fabSpecs,setFabSpecs]=useState([mkFab()]);
  ...
  // Print spec helpers
  const updatePrint=(idx,field,val)=>setPrintSpecs(prev=>prev.map((s,i)=>i===idx?{...s,[field]:val}:s));
  const addPrint=()=>setPrintSpecs(prev=>[...prev,mkPrint()]);
  const removePrint=(idx)=>setPrintSpecs(prev=>prev.filter((_,i)=>i!==idx));

  // Fab spec helpers
  const updateFab=(idx,field,val)=>setFabSpecs(prev=>prev.map((s,i)=>i===idx?{...s,[field]:val}:s));
  const addFab=()=>setFabSpecs(prev=>[...prev,mkFab()]);
  const removeFab=(idx)=>setFabSpecs(prev=>prev.filter((_,i)=>i!==idx));

  const getFabFields=(spec)=>{
    if(spec.type==="Wooden Frame") return [["woodType","Wood Type",WOOD_TYPES]];
    if(spec.type==="ACP Base") return [["acpThick","ACP Thickness",ACP_THICK],["pipeSize","Frame Pipe Size",PIPE_SIZES],["pipeThick","Pipe Thickness",PIPE_THICK]];
  ...
  const calcLine=(s)=>{
    ...
    const q=parseFloat(s.qty)||1;
    const r=parseFloat(s.rate)||0;
    return w*h*q*r;
  };
  const printTotal=form.needsPrint?printSpecs.reduce((s,p)=>s+calcLine(p),0):0;
  const fabTotal=form.needsFabrication?fabSpecs.reduce((s,f)=>s+calcLine(f),0):0;
  const autoTotal=printTotal+fabTotal;
  const displayTotal=form.amountManual?form.amount:(autoTotal>0?String(autoTotal):form.amount);
  ...
  function submit(){
    ...
    if(!cn||!form.jobDesc||!form.deadline){showToast("Fill all required fields","error");return;}
    const finalAmount=parseFloat(displayTotal)||0;
    if(finalAmount<=0){showToast("Amount must be > 0","error");return;}
    const orderId=genOrderId(jobs);
    const newJob={...form,custId:cid,clientName:cn,orderId,amount:finalAmount,paid:parseFloat(form.paid)||0,createdAt:new Date().toISOString().slice(0,10),deliveredAt:null,
      printSpec:form.needsPrint?printSpecs[0]:null,
      fabSpec:form.needsFabrication?fabSpecs[0]:null,
      printSpecs:form.needsPrint?printSpecs:null,
      fabSpecs:form.needsFabrication?fabSpecs:null,
    };
    setJobs(prev=>[...prev,newJob]);
    // Create one print sub-job per print spec
    if(form.needsPrint){
      printSpecs.forEach((ps,idx)=>{
        const suffix=printSpecs.length>1?"-"+(idx+1):"";
        const pj={id:"PJ-"+orderId+suffix,orderId,clientName:cn,jobDesc:form.jobDesc,type:ps.type,material:ps.material,width:ps.width,height:ps.height,qty:ps.qty,resolution:ps.resolution,notes:ps.notes,stage:"Queued",createdAt:new Date().toISOString().slice(0,10)};
        setPrintJobs(prev=>[...prev,pj]);
      });
    }
    // Create one fab sub-job per fab spec
    if(form.needsFabrication){
      fabSpecs.forEach((fs,idx)=>{
        const suffix=fabSpecs.length>1?"-"+(idx+1):"";
        const fj={id:"FAB-"+orderId+suffix,orderId,clientName:cn,jobDesc:form.jobDesc,type:fs.type,pipeSize:fs.pipeSize,pipeThick:fs.pipeThick,woodType:fs.woodType,acpThick:fs.acpThick,width:fs.width,height:fs.height,notes:fs.notes,stage:"Pending",createdAt:new Date().toISOString().slice(0,10)};
    ...
    const printCount=form.needsPrint?printSpecs.length:0;
    const fabCount=form.needsFabrication?fabSpecs.length:0;
    showToast(orderId+" created!"+(printCount?" "+printCount+" print job(s).":"")+(fabCount?" "+fabCount+" fab job(s).":""));
    setForm(blank);setPrintSpecs([mkPrint()]);setFabSpecs([mkFab()]);setNc({name:"",phone:"",email:"",gst:"",address:"",monthlyBilling:false});setNewClient(false);setPage("pending");
  }
    ...
    <div style={S.content}>
      <div style={S.formCard}>
        ...
        <div style={S.formGrid}>
          ...
          <div style={S.fg}><label style={S.label}>Order Type *</label><select style={S.input} value={form.orderType} onChange={e=>setForm({...form,orderType:e.target.value})}>{orderTypes.map(t=><option key={t}>{t}</option>)}</select></div>
        ...
        {/* Print specs with pricing */}
        {form.needsPrint&&(
          <div style={{marginTop:12,padding:14,background:"#0d1a2e",borderRadius:9,border:"1px solid #4f8ef733"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:11,color:"#4f8ef7",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>🖨 Print Specifications ({printSpecs.length})</div>
              <button onClick={addPrint} style={{background:"#4f8ef722",color:"#4f8ef7",border:"1px solid #4f8ef744",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:700}}>+ Add Size</button>
            </div>
            {printSpecs.map((ps,idx)=>{const lineAmt=calcLine(ps); return (
              <div key={ps._id} style={{background:"#0a1420",borderRadius:8,padding:12,marginBottom:8,border:"1px solid #1a2a3e"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontSize:10,color:"#4f8ef7",fontWeight:700}}>Print #{idx+1}{ps.width&&ps.height?" — "+ps.width+"×"+ps.height+"ft":""}</span>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    {lineAmt>0&&<span style={{fontSize:11,color:"#27c27c",fontWeight:700}}>₹{lineAmt.toLocaleString()}</span>}
                    {printSpecs.length>1&&<button onClick={()=>removePrint(idx)} style={{background:"none",border:"1px solid #ff3b3b33",color:"#ff3b3b",borderRadius:4,padding:"2px 7px",cursor:"pointer",fontSize:9}}>✕</button>}
                  </div>
                </div>
                <div style={S.formGrid}>
                  <div style={S.fg}><label style={S.label}>Print Type</label><select style={S.input} value={ps.type} onChange={e=>updatePrint(idx,"type",e.target.value)}>{PRINT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
                  <div style={S.fg}><label style={S.label}>Material</label><select style={S.input} value={ps.material} onChange={e=>updatePrint(idx,"material",e.target.value)}>{PRINT_MATS.map(m=><option key={m}>{m}</option>)}</select></div>
                  <div style={S.fg}><label style={S.label}>Width (ft)</label><input type="number" style={S.input} value={ps.width} onChange={e=>updatePrint(idx,"width",e.target.value)} placeholder="0"/></div>
                  <div style={S.fg}><label style={S.label}>Height (ft)</label><input type="number" style={S.input} value={ps.height} onChange={e=>updatePrint(idx,"height",e.target.value)} placeholder="0"/></div>
                  <div style={S.fg}><label style={S.label}>Qty</label><input type="number" style={S.input} value={ps.qty} onChange={e=>updatePrint(idx,"qty",e.target.value)} placeholder="1"/></div>
                  <div style={S.fg}><label style={S.label}>Rate ₹/sqft</label><input type="number" style={S.input} value={ps.rate} onChange={e=>updatePrint(idx,"rate",e.target.value)} placeholder="e.g. 25"/></div>
                  <div style={S.fg}><label style={S.label}>Resolution</label><select style={S.input} value={ps.resolution} onChange={e=>updatePrint(idx,"resolution",e.target.value)}>{["360dpi","720dpi","1080dpi","1440dpi","2880dpi"].map(r=><option key={r}>{r}</option>)}</select></div>
                  <div style={S.fg}><label style={S.label}>Notes</label><input style={S.input} value={ps.notes} onChange={e=>updatePrint(idx,"notes",e.target.value)} placeholder="Lamination, etc."/></div>
                </div>
                {lineAmt>0&&<div style={{marginTop:6,fontSize:10,color:"#607080",textAlign:"right"}}>{ps.width}×{ps.height}ft × {ps.qty||1} pcs × ₹{ps.rate}/sqft = <span style={{color:"#27c27c",fontWeight:700}}>₹{lineAmt.toLocaleString()}</span></div>}
              </div>
            );})}
            <div style={{textAlign:"center",paddingTop:4}}><button onClick={addPrint} style={{background:"none",border:"1px dashed #4f8ef744",color:"#4f8ef7",borderRadius:6,padding:"6px 16px",cursor:"pointer",fontSize:10}}>+ Add Another Print Size</button></div>
            {printTotal>0&&<div style={{marginTop:8,padding:"8px 12px",background:"#0a1420",borderRadius:7,display:"flex",justifyContent:"space-between",border:"1px solid #4f8ef733"}}><span style={{fontSize:11,color:"#a0b8c8"}}>Print Subtotal</span><span style={{fontSize:13,color:"#4f8ef7",fontWeight:800}}>₹{printTotal.toLocaleString()}</span></div>}
          </div>
        )}

        {/* Fab specs with pricing */}
        {form.needsFabrication&&(
          <div style={{marginTop:12,padding:14,background:"#1a0d00",borderRadius:9,border:"1px solid #ff8c0033"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:11,color:"#ff8c00",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>🔧 Fabrication Specifications ({fabSpecs.length})</div>
              <button onClick={addFab} style={{background:"#ff8c0022",color:"#ff8c00",border:"1px solid #ff8c0044",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:700}}>+ Add Size</button>
            </div>
            {fabSpecs.map((fs,idx)=>{const lineAmt=calcLine(fs); return (
              <div key={fs._id} style={{background:"#120a00",borderRadius:8,padding:12,marginBottom:8,border:"1px solid #2a1a00"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontSize:10,color:"#ff8c00",fontWeight:700}}>Frame #{idx+1}{fs.width&&fs.height?" — "+fs.width+"×"+fs.height+"ft "+fs.type:""}</span>
                  <div style={{display:"flex",gap:6,alignItems:"center"}}>
                    {lineAmt>0&&<span style={{fontSize:11,color:"#27c27c",fontWeight:700}}>₹{lineAmt.toLocaleString()}</span>}
                    {fabSpecs.length>1&&<button onClick={()=>removeFab(idx)} style={{background:"none",border:"1px solid #ff3b3b33",color:"#ff3b3b",borderRadius:4,padding:"2px 7px",cursor:"pointer",fontSize:9}}>✕</button>}
                  </div>
                </div>
                <div style={S.formGrid}>
                  <div style={S.fg}><label style={S.label}>Frame Type</label><select style={S.input} value={fs.type} onChange={e=>updateFab(idx,"type",e.target.value)}>{FAB_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
                  {getFabFields(fs).map(arr=> (
                    <div key={arr[0]} style={S.fg}><label style={S.label}>{arr[1]}</label><select style={S.input} value={fs[arr[0]]} onChange={e=>updateFab(idx,arr[0],e.target.value)}>{arr[2].map(o=><option key={o}>{o}</option>)}</select></div>
                  ))}
                  <div style={S.fg}><label style={S.label}>Width (ft)</label><input type="number" style={S.input} value={fs.width} onChange={e=>updateFab(idx,"width",e.target.value)} placeholder="0"/></div>
                  <div style={S.fg}><label style={S.label}>Height (ft)</label><input type="number" style={S.input} value={fs.height} onChange={e=>updateFab(idx,"height",e.target.value)} placeholder="0"/></div>
                  <div style={S.fg}><label style={S.label}>Qty</label><input type="number" style={S.input} value={fs.qty} onChange={e=>updateFab(idx,"qty",e.target.value)} placeholder="1"/></div>
                  <div style={S.fg}><label style={S.label}>Rate ₹/sqft</label><input type="number" style={S.input} value={fs.rate} onChange={e=>updateFab(idx,"rate",e.target.value)} placeholder="e.g. 150"/></div>
                  <div style={{...S.fg,gridColumn:"1/-1"}}><label style={S.label}>Notes</label><input style={S.input} value={fs.notes} onChange={e=>updateFab(idx,"notes",e.target.value)} placeholder="Finish, coating, etc."/></div>
                </div>
                {lineAmt>0&&<div style={{marginTop:6,fontSize:10,color:"#607080",textAlign:"right"}}>{fs.width}×{fs.height}ft × {fs.qty||1} pcs × ₹{fs.rate}/sqft = <span style={{color:"#27c27c",fontWeight:700}}>₹{lineAmt.toLocaleString()}</span></div>}
              </div>
            );})}
            <div style={{textAlign:"center",paddingTop:4}}><button onClick={addFab} style={{background:"none",border:"1px dashed #ff8c0044",color:"#ff8c00",borderRadius:6,padding:"6px 16px",cursor:"pointer",fontSize:10}}>+ Add Another Frame Size</button></div>
            {fabTotal>0&&<div style={{marginTop:8,padding:"8px 12px",background:"#120a00",borderRadius:7,display:"flex",justifyContent:"space-between",border:"1px solid #ff8c0033"}}><span style={{fontSize:11,color:"#a0b8c8"}}>Fabrication Subtotal</span><span style={{fontSize:13,color:"#ff8c00",fontWeight:800}}>₹{fabTotal.toLocaleString()}</span></div>}
          </div>
        )}

        {/* Pricing summary */}
        {autoTotal>0&&(
          <div style={{marginTop:12,padding:"12px 14px",background:"#0a1f0a",borderRadius:9,border:"1px solid #27c27c33"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontSize:12,color:"#27c27c",fontWeight:700}}>Auto-calculated Total</div>
                <div style={{fontSize:9,color:"#607080",marginTop:2}}>{form.needsPrint?printSpecs.length+" print":""}{form.needsPrint&&form.needsFabrication?" + ":""}{form.needsFabrication?fabSpecs.length+" fab":""} items</div>
              </div>
              <div style={{fontSize:20,color:"#27c27c",fontWeight:800}}>₹{autoTotal.toLocaleString()}</div>
            </div>
          </div>
        )}

        {/* Urgency, Stage, Amount, Advance, Notes */}
        <div style={{marginTop:14,padding:14,background:"#0d1520",borderRadius:9,border:"1px solid #1a2535"}}>
          <div style={{fontSize:10,color:"#607080",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:10}}>Job Settings & Pricing</div>
          <div style={S.formGrid}>
            ...
            <div style={S.fg}>
              <label style={S.label}>Total Amount ₹ {autoTotal>0&&<span style={{color:"#27c27c",fontWeight:400,textTransform:"none"}}>(auto: ₹{autoTotal.toLocaleString()})</span>}</label>
              <input type="number" style={S.input} value={displayTotal} onChange={e=>setForm({...form,amount:e.target.value,amountManual:true})} placeholder="0"/>
              {autoTotal>0&&form.amountManual&&<button onClick={()=>setForm(f=>({...f,amount:"",amountManual:false}))} style={{fontSize:9,color:"#4f8ef7",background:"none",border:"none",cursor:"pointer",marginTop:2,padding:0}}>↺ Use auto-calculated</button>}
            </div>
            ...
            <div style={{...S.fg,gridColumn:"1/-1"}}><label style={S.label}>Notes</label><textarea style={{...S.input,minHeight:44,resize:"vertical"}} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Special instructions, materials, delivery notes…"/></div>
          </div>
        </div>
...
// ── Pending Jobs ───────────────────────────────────────────────────
function PendingJobs({jobs,setJobs,showToast,stages,deleteJob,orderTypes,printJobs,fabJobs}){
  ...
  return(
    <div style={S.content}>
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <select style={{...S.input,maxWidth:170}} value={filter} onChange={e=>setFilter(e.target.value)}><option>All</option>{orderTypes.map(t=><option key={t}>{t}</option>)}</select>
      ...
      {[{label:"🔴 URGENT — ≤ 3 days",list:red,border:"#ff3b3b"},{label:"🟡 SOON — 4–5 days",list:yellow,border:"#f5a623"},{label:"🟢 ON TRACK — 6+ days",list:green,border:"#27c27c"}].map(sec=>sec.list.length>0&&(
        <div key={sec.label} style={{...S.card,borderLeft:`4px solid ${sec.border}`,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:9}}>
            {sec.list.map(j=><PCard key={j.orderId} job={j} border={sec.border} expanded={expandedId===j.orderId} setExpanded={setExpandedId} markDelivered={markDelivered} updateStage={updateStage} updatePaid={updatePaid} updateDeadline={updateDeadline} stages={stages} deleteJob={deleteJob} printJobs={printJobs} fabJobs={fabJobs}/>)}
          </div>
        </div>
      ))}
...
function PCard({job,border,expanded,setExpanded,markDelivered,updateStage,updatePaid,updateDeadline,stages,deleteJob,printJobs,fabJobs}){
  const[confirmDel,setConfirmDel]=useState(false);
  const days=daysUntil(job.deadline),due=job.amount-job.paid,imgs=job.images||[];
  const pJobs=(printJobs||[]).filter(p=>p.orderId===job.orderId);
  const fJobs=(fabJobs||[]).filter(f=>f.orderId===job.orderId);
  return(
      ...
      {expanded&&(
        <div style={{marginTop:9,paddingTop:9,borderTop:"1px solid #1a2535"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:7}}>
            {[{l:"Type",v:job.orderType},{l:"Amount",v:"₹"+job.amount.toLocaleString()},{l:"Paid",v:"₹"+job.paid.toLocaleString()},{l:"Balance",v:due>0?"₹"+due.toLocaleString():"Paid ✓"},{l:"Created",v:job.createdAt||"—"},{l:"Deadline",v:job.deadline||"—"}].map(k=> (
              <div key={k.l} style={{background:"#1a2535",borderRadius:5,padding:"4px 7px"}}><div style={{fontSize:8,color:"#405060",fontWeight:700,textTransform:"uppercase"}}>{k.l}</div><div style={{fontSize:11,color:"#c8d8e8",fontWeight:600}}>{k.v}</div></div>
            ))}
          </div>
          {job.notes&&<div style={{fontSize:10,color:"#607080",marginBottom:6,fontStyle:"italic"}}>📝 {job.notes}</div>}
          {/* Print specs */}
          {pJobs.length>0&&<div style={{marginBottom:6}}><div style={{fontSize:9,color:"#4f8ef7",fontWeight:700,marginBottom:3}}>🖨 Print ({pJobs.length})</div>{pJobs.map(p=> (<div key={p.id} style={{fontSize:10,color:"#a0b8c8",padding:"3px 7px",background:"#0a1420",borderRadius:4,marginBottom:2,border:"1px solid #1a2a3e"}}>{p.type} · {p.material} · {p.width||"?"}×{p.height||"?"}ft × {p.qty||1} · <span style={{color:PRINT_STAGE_COLORS[p.stage]||"#607080"}}>{p.stage}</span></div>))}</div>}
          {/* Fab specs */}
          {fJobs.length>0&&<div style={{marginBottom:6}}><div style={{fontSize:9,color:"#ff8c00",fontWeight:700,marginBottom:3}}>🔧 Fab ({fJobs.length})</div>{fJobs.map(f=> (<div key={f.id} style={{fontSize:10,color:"#a0b8c8",padding:"3px 7px",background:"#120a00",borderRadius:4,marginBottom:2,border:"1px solid #2a1a00"}}>{f.type} · {f.width||"?"}×{f.height||"?"}ft · <span style={{color:FAB_STAGE_COLORS[f.stage]||"#607080"}}>{f.stage}</span></div>))}</div>}
          ...
          {!confirmDel?<button onClick={()=>setConfirmDel(true)} style={{width:"100%",background:"none",border:"1px solid #ff3b3b33",color:"#ff3b3b55",borderRadius:5,padding:"6px",fontSize:10,cursor:"pointer",marginTop:5}}>🗑 Delete Job</button>
          :<button onClick={()=>{deleteJob(job.orderId);}} style={{width:"100%",background:"#ff3b3b22",border:"1px solid #ff3b3b",color:"#ff3b3b",borderRadius:5,padding:"6px",fontSize:10,cursor:"pointer",fontWeight:700,marginTop:5}}>⚠ Confirm Delete</button>}
        </div>
      )}
    </div>
  );
...
function PrintDept({printJobs,setPrintJobs,jobs,showToast}){
  ...
  const[addForm,setAddForm]=useState({orderId:"",type:"Digital Flex",material:"Star Flex",width:"",height:"",qty:"1",resolution:"720dpi",notes:""});

  const filtered=useMemo(()=>{
    let j=[...printJobs];
    if(!showDone&&stageFilter==="All")j=j.filter(x=>x.stage!=="Done");
    ...
    return j;
  },[printJobs,search,stageFilter,showDone]);

  function updateStage(id,stage){setPrintJobs(prev=>prev.map(p=>p.id===id?{...p,stage}:p));showToast("Print stage updated");}
  function deletePrint(id){setPrintJobs(prev=>prev.filter(p=>p.id!==id));showToast("Print job removed");}
  function addPrintJob(){
    if(!addForm.orderId||!addForm.width||!addForm.height){showToast("Select order + size","error"); return;}
    const parent=jobs.find(j=>j.orderId===addForm.orderId);
    const count=printJobs.filter(p=>p.orderId===addForm.orderId).length;
    const id="PJ-"+addForm.orderId+"-"+(count+1);
    setPrintJobs(prev=>[...prev,{id,orderId:addForm.orderId,clientName:parent?parent.clientName:"",jobDesc:parent?parent.jobDesc:"",type:addForm.type,material:addForm.material,width:addForm.width,height:addForm.height,qty:addForm.qty,resolution:addForm.resolution,notes:addForm.notes,stage:"Queued",createdAt:new Date().toISOString().slice(0,10)}]);
    showToast("Print job added: "+addForm.width+"×"+addForm.height+"ft");
    setAddForm({orderId:addForm.orderId,type:addForm.type,material:addForm.material,width:"",height:"",qty:"1",resolution:addForm.resolution,notes:""});
  }
  ...
  return(
    <div style={S.content}>
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button onClick={()=>setShowAdd(!showAdd)} style={{...S.submitBtn,padding:"6px 14px",fontSize:11,background:showAdd?"#ff3b3b22":"#4f8ef7",color:showAdd?"#ff3b3b":"#fff",border:showAdd?"1px solid #ff3b3b44":"none"}}>{showAdd?"✕ Close":"+ Add Print Size"}</button>
        <button onClick={()=>setShowDone(!showDone)} style={{padding:"6px 12px",fontSize:10,background:showDone?"#27c27c22":"#0d1520",color:showDone?"#27c27c":"#607080",border:"1px solid "+(showDone?"#27c27c44":"#1a2535"),borderRadius:6,cursor:"pointer"}}>{showDone?"Hide Done ✓":"Show Done ("+printJobs.filter(p=>p.stage==="Done").length+")"}</button>
      ...
      {showAdd&&(
        <div style={{...S.card,borderLeft:"4px solid #4f8ef7",marginBottom:14}}>
          <h3 style={S.cardTitle}>Add Print Size / Variant</h3>
          <div style={S.formGrid}>
            <div style={S.fg}><label style={S.label}>Order *</label><select style={S.input} value={addForm.orderId} onChange={e=>setAddForm({...addForm,orderId:e.target.value})}><option value="">Select order...</option>{jobs.filter(j=>j.stage!=="Delivered").map(j=><option key={j.orderId} value={j.orderId}>{j.orderId} — {j.clientName}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Print Type</label><select style={S.input} value={addForm.type} onChange={e=>setAddForm({...addForm,type:e.target.value})}>{PRINT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Material</label><select style={S.input} value={addForm.material} onChange={e=>setAddForm({...addForm,material:e.target.value})}>{PRINT_MATS.map(m=><option key={m}>{m}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Width (ft) *</label><input type="number" style={S.input} value={addForm.width} onChange={e=>setAddForm({...addForm,width:e.target.value})} placeholder="0"/></div>
            <div style={S.fg}><label style={S.label}>Height (ft) *</label><input type="number" style={S.input} value={addForm.height} onChange={e=>setAddForm({...addForm,height:e.target.value})} placeholder="0"/></div>
            <div style={S.fg}><label style={S.label}>Qty</label><input type="number" style={S.input} value={addForm.qty} onChange={e=>setAddForm({...addForm,qty:e.target.value})} placeholder="1"/></div>
            <div style={S.fg}><label style={S.label}>Resolution</label><select style={S.input} value={addForm.resolution} onChange={e=>setAddForm({...addForm,resolution:e.target.value})}>{["360dpi","720dpi","1080dpi","1440dpi","2880dpi"].map(r=><option key={r}>{r}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Notes</label><input style={S.input} value={addForm.notes} onChange={e=>setAddForm({...addForm,notes:e.target.value})} placeholder="Lamination, etc."/></div>
          </div>
          <button onClick={addPrintJob} style={{...S.submitBtn,marginTop:10}}>+ Add Print Job</button>
        </div>
      )}

      {printJobs.length===0&&!showAdd&&<div style={S.empty}>No print jobs yet. They auto-create when a job with "Requires Print" is added.</div>}
...
function FabDept({fabJobs,setFabJobs,jobs,showToast}){
  ...
  const[showAdd,setShowAdd]=useState(false);
  const[showDone,setShowDone]=useState(false);
  const[af,setAf]=useState({orderId:"",type:"MS Frame",pipeSize:"1\"",pipeThick:"2mm",woodType:"Plywood 18mm",acpThick:"4mm",width:"",height:"",notes:""});

  const filtered=useMemo(()=>{
    let j=[...fabJobs];
    if(!showDone&&stageFilter==="All")j=j.filter(x=>x.stage!=="Dispatched");
    if(stageFilter!=="All")j=j.filter(x=>x.stage===stageFilter);
    ...
    return j;
  },[fabJobs,search,stageFilter,typeFilter,showDone]);

  function updateStage(id,stage){setFabJobs(prev=>prev.map(f=>f.id===id?{...f,stage}:f));showToast("Fab stage updated");}
  function deleteFab(id){setFabJobs(prev=>prev.filter(f=>f.id!==id));showToast("Fab job removed");}
  function addFabJob(){
    if(!af.orderId||!af.width||!af.height){showToast("Select order + size","error"); return;}
    const parent=jobs.find(j=>j.orderId===af.orderId);
    const count=fabJobs.filter(f=>f.orderId===af.orderId).length;
    const id="FAB-"+af.orderId+"-"+(count+1);
    setFabJobs(prev=>[...prev,{id,orderId:af.orderId,clientName:parent?parent.clientName:"",jobDesc:parent?parent.jobDesc:"",type:af.type,pipeSize:af.pipeSize,pipeThick:af.pipeThick,woodType:af.woodType,acpThick:af.acpThick,width:af.width,height:af.height,notes:af.notes,stage:"Pending",createdAt:new Date().toISOString().slice(0,10)}]);
    showToast("Fab job added: "+af.width+"×"+af.height+"ft "+af.type);
    setAf({...af,width:"",height:"",notes:""});
  }
  ...
  return(
    <div style={S.content}>
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <button onClick={()=>setShowAdd(!showAdd)} style={{...S.submitBtn,padding:"6px 14px",fontSize:11,background:showAdd?"#ff3b3b22":"#ff8c00",color:showAdd?"#ff3b3b":"#fff",border:showAdd?"1px solid #ff3b3b44":"none"}}>{showAdd?"✕ Close":"+ Add Fab Size"}</button>
        <button onClick={()=>setShowDone(!showDone)} style={{padding:"6px 12px",fontSize:10,background:showDone?"#00c9a722":"#0d1520",color:showDone?"#00c9a7":"#607080",border:"1px solid "+(showDone?"#00c9a744":"#1a2535"),borderRadius:6,cursor:"pointer"}}>{showDone?"Hide Done ✓":"Show Done ("+fabJobs.filter(f=>f.stage==="Dispatched").length+")"}</button>
      ...
      {showAdd&&(
        <div style={{...S.card,borderLeft:"4px solid #ff8c00",marginBottom:14}}>
          <h3 style={S.cardTitle}>Add Fabrication Size / Variant</h3>
          <div style={S.formGrid}>
            <div style={S.fg}><label style={S.label}>Order *</label><select style={S.input} value={af.orderId} onChange={e=>setAf({...af,orderId:e.target.value})}><option value="">Select order...</option>{jobs.filter(j=>j.stage!=="Delivered").map(j=><option key={j.orderId} value={j.orderId}>{j.orderId} — {j.clientName}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Frame Type</label><select style={S.input} value={af.type} onChange={e=>setAf({...af,type:e.target.value})}>{FAB_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Pipe Size</label><select style={S.input} value={af.pipeSize} onChange={e=>setAf({...af,pipeSize:e.target.value})}>{PIPE_SIZES.map(p=><option key={p}>{p}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Pipe Thickness</label><select style={S.input} value={af.pipeThick} onChange={e=>setAf({...af,pipeThick:e.target.value})}>{PIPE_THICK.map(t=><option key={t}>{t}</option>)}</select></div>
            <div style={S.fg}><label style={S.label}>Width (ft) *</label><input type="number" style={S.input} value={af.width} onChange={e=>setAf({...af,width:e.target.value})} placeholder="0"/></div>
            <div style={S.fg}><label style={S.label}>Height (ft) *</label><input type="number" style={S.input} value={af.height} onChange={e=>setAf({...af,height:e.target.value})} placeholder="0"/></div>
            <div style={{...S.fg,gridColumn:"1/-1"}}><label style={S.label}>Notes</label><input style={S.input} value={af.notes} onChange={e=>setAf({...af,notes:e.target.value})} placeholder="Finish, coating, etc."/></div>
          </div>
          <button onClick={addFabJob} style={{...S.submitBtn,marginTop:10,background:"#ff8c00"}}>+ Add Fab Job</button>
        </div>
      )}

      {fabJobs.length===0&&!showAdd&&<div style={S.empty}>No fabrication jobs yet. They auto-create when a job with "Requires Fabrication" is added.</div>}
...
// ── Pending Receivables ───────────────────────────────────────────
function Receivables({jobs,setJobs,clients,paymentAlerts,showToast}){
  const[expandedId,setExpandedId]=useState(null);
  function updatePaid(orderId,paid){setJobs(prev=>prev.map(j=>j.orderId===orderId?{...j,paid:parseFloat(paid)||0}:j));showToast("Payment updated");}
  ...
  const fresh=paymentAlerts.filter(x=>x.payLevel==="new");
  const totalDue=paymentAlerts.reduce((s,j)=>s+(j.amount-j.paid),0);
  const kpis=[{l:"Critical (10d+)",v:red.length,c:"#ff3b3b"},{l:"Overdue (5-10d)",v:orange.length,c:"#ff8c00"},{l:"Due (2-5d)",v:yellow.length,c:"#f5a623"},{l:"Recent",v:fresh.length,c:"#4f8ef7"},{l:"Total Pending",v:"₹"+totalDue.toLocaleString(),c:"#c27ee8"}];
  const levelConfig=[{label:"🔴 Critical — 10+ days",list:red,color:"#ff3b3b"},{label:"🟠 Overdue — 5–10 days",list:orange,color:"#ff8c00"},{label:"🟡 Due — 2–5 days",list:yellow,color:"#f5a623"},{label:"🔵 Recent — Balance pending",list:fresh,color:"#4f8ef7"}];
  if(paymentAlerts.length===0) return (<div style={S.content}><div style={S.empty}>🎉 No pending receivables!</div></div>);
  return (
    <div style={S.content}>
      <div style={S.kpiRow}>
        {kpis.map(k=> (
          <div key={k.l} style={{...S.kpiCard,borderTop:"3px solid "+k.c}}><span style={{fontSize:18,fontWeight:800,color:k.c}}>{k.v}</span><span style={{fontSize:9,color:"#607080"}}>{k.l}</span></div>
        ))}
      </div>
              ...
              <div key={j.orderId} style={{background:"#0d1520",borderRadius:9,padding:11,border:"1px solid #1a2535",borderTop:"3px solid "+sec.color}}>
                <div onClick={()=>setExpandedId(exp?null:j.orderId)} style={{cursor:"pointer"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={S.orderBadge}>{j.orderId}</span>
                    <span style={{fontSize:9,background:sec.color+"22",color:sec.color,padding:"2px 7px",borderRadius:9,fontWeight:700}}>{j.daysAfterDelivery>0?j.daysAfterDelivery+"d":"New"}</span>
                  </div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:9,background:"#1a2a3e",color:"#a0b8ff",padding:"2px 6px",borderRadius:10}}>{j.orderType}</span>
                    <span style={{color:"#ff3b3b",fontSize:12,fontWeight:800}}>₹{due.toLocaleString()}</span>
                  </div>
                ...
                {exp&&<div style={{marginTop:8,paddingTop:8,borderTop:"1px solid #1a2535"}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4,marginBottom:7}}>
                    {[{l:"Total",v:"₹"+j.amount.toLocaleString(),c:"#a0b8c8"},{l:"Paid",v:"₹"+j.paid.toLocaleString(),c:"#27c27c"},{l:"Balance",v:"₹"+due.toLocaleString(),c:"#ff3b3b"}].map(k=> (
                      <div key={k.l} style={{background:"#1a2535",borderRadius:5,padding:"5px 7px",textAlign:"center"}}><div style={{fontSize:12,fontWeight:700,color:k.c}}>{k.v}</div><div style={{fontSize:8,color:"#607080",marginTop:1}}>{k.l}</div></div>
                    ))}
                  </div>
                  <label style={{...S.label,marginBottom:3,display:"block"}}>Record Payment ₹</label>
                  <input type="number" style={{...S.input,padding:"6px 9px",marginBottom:4}} defaultValue={j.paid} onBlur={e=>updatePaid(j.orderId,e.target.value)}/>
                  <button onClick={()=>updatePaid(j.orderId,j.amount)} style={{width:"100%",background:"#27c27c22",color:"#27c27c",border:"1px solid #27c27c55",borderRadius:5,padding:"5px",fontSize:10,cursor:"pointer",fontWeight:700}}>✓ Mark Fully Paid</button>
                  <div style={{fontSize:9,color:"#405060",marginTop:4}}>Stage: {j.stage} · {j.deliveredAt?"Delivered: "+j.deliveredAt:"Created: "+j.createdAt}</div>
                </div>}
              </div>
            );})}
          </div>
...
// ── Completed Jobs ────────────────────────────────────────────────
function CompletedJobs({jobs,setJobs,showToast,printJobs,fabJobs}){
  const[search,setSearch]=useState("");
  const[expandedId,setExpandedId]=useState(null);
  const completed=useMemo(()=>{
    let j=jobs.filter(j=>j.stage==="Delivered");
    if(search)j=j.filter(x=>(x.clientName||"").toLowerCase().includes(search.toLowerCase())||(x.orderId||"").toLowerCase().includes(search.toLowerCase())||(x.jobDesc||"").toLowerCase().includes(search.toLowerCase()));
    return j.sort((a,b)=>(b.deliveredAt||b.createdAt||"").localeCompare(a.deliveredAt||a.createdAt||""));
  },[jobs,search]);
  const totalRev=completed.reduce((s,j)=>s+j.amount,0);
  const totalPaid=completed.reduce((s,j)=>s+j.paid,0);
  const totalDue=totalRev-totalPaid;
  const kpis=[{l:"Completed",v:completed.length,c:"#27c27c"},{l:"Revenue",v:"₹"+totalRev.toLocaleString(),c:"#4f8ef7"},{l:"Collected",v:"₹"+totalPaid.toLocaleString(),c:"#27c27c"},{l:"Pending",v:"₹"+totalDue.toLocaleString(),c:totalDue>0?"#ff3b3b":"#27c27c"}];
  return (
    <div style={S.content}>
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        <input style={{...S.input,maxWidth:260}} placeholder="🔍 Search completed jobs…" value={search} onChange={e=>setSearch(e.target.value)}/>
        <span style={{marginLeft:"auto",fontSize:11,color:"#607080"}}>{completed.length} job{completed.length!==1?"s":""}</span>
      </div>
      <div style={S.kpiRow}>
        {kpis.map(k=> (
          <div key={k.l} style={{...S.kpiCard,borderTop:"3px solid "+k.c}}><span style={{fontSize:18,fontWeight:800,color:k.c}}>{k.v}</span><span style={{fontSize:9,color:"#607080"}}>{k.l}</span></div>
      ...
      {completed.length===0&&<div style={S.empty}>No completed jobs yet</div>}
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:9}}>
        {completed.map(j=>{const due=j.amount-j.paid;const exp=expandedId===j.orderId;const pJobs=printJobs.filter(p=>p.orderId===j.orderId);const fJobs=fabJobs.filter(f=>f.orderId===j.orderId); return (
          <div key={j.orderId} style={{background:"#0d1520",borderRadius:9,padding:12,border:"1px solid #1a2535",borderTop:"3px solid #27c27c"}}>
            <div onClick={()=>setExpandedId(exp?null:j.orderId)} style={{cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                <span style={S.orderBadge}>{j.orderId}</span>
                <span style={{fontSize:9,color:"#27c27c",fontWeight:700}}>{j.deliveredAt||"Delivered"}</span>
              </div>
              <div style={{fontWeight:700,color:"#e0e8f5",fontSize:12,marginBottom:2}}>{j.clientName}</div>
              <div style={{fontSize:10,color:"#607080",marginBottom:4}}>{j.jobDesc}</div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:9,background:"#1a2a3e",color:"#a0b8ff",padding:"2px 6px",borderRadius:10}}>{j.orderType}</span>
                <div style={{display:"flex",gap:6}}>
                  <span style={{fontSize:11,color:"#4f8ef7",fontWeight:700}}>₹{j.amount.toLocaleString()}</span>
                  {due>0&&<span style={{fontSize:11,color:"#ff3b3b",fontWeight:700}}>₹{due.toLocaleString()} due</span>}
                </div>
              </div>
              <div style={{fontSize:9,color:"#304050",textAlign:"center",marginTop:4}}>{exp?"▲ hide details":"▼ view details"}</div>
            </div>
            {exp&&<div style={{marginTop:8,paddingTop:8,borderTop:"1px solid #1a2535"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,marginBottom:7}}>
                {[{l:"Amount",v:"₹"+j.amount.toLocaleString()},{l:"Paid",v:"₹"+j.paid.toLocaleString()},{l:"Created",v:j.createdAt||"—"},{l:"Delivered",v:j.deliveredAt||"—"}].map(k=> (
                  <div key={k.l} style={{background:"#1a2535",borderRadius:5,padding:"5px 7px"}}><div style={{fontSize:8,color:"#405060",fontWeight:700,textTransform:"uppercase"}}>{k.l}</div><div style={{fontSize:11,color:"#c8d8e8"}}>{k.v}</div></div>
                ))}
              </div>
              {j.notes&&<div style={{fontSize:10,color:"#607080",marginBottom:5,fontStyle:"italic"}}>📝 {j.notes}</div>}
              {pJobs.length>0&&<div style={{marginBottom:5}}><div style={{fontSize:9,color:"#4f8ef7",fontWeight:700,marginBottom:3}}>🖨 Print Jobs ({pJobs.length})</div>{pJobs.map(p=> (<div key={p.id} style={{fontSize:10,color:"#a0b8c8",padding:"3px 7px",background:"#1a2535",borderRadius:4,marginBottom:2}}>{p.type} · {p.material} · {p.width||"?"}×{p.height||"?"}ft · {p.stage}</div>))}</div>}
              {fJobs.length>0&&<div><div style={{fontSize:9,color:"#ff8c00",fontWeight:700,marginBottom:3}}>🔧 Fab Jobs ({fJobs.length})</div>{fJobs.map(f=> (<div key={f.id} style={{fontSize:10,color:"#a0b8c8",padding:"3px 7px",background:"#1a2535",borderRadius:4,marginBottom:2}}>{f.type} · {f.width||"?"}×{f.height||"?"}ft · {f.stage}</div>))}</div>}
            </div>}
          </div>
        );})}
      </div>
...
// ── Reports ────────────────────────────────────────────────────────
function Reports({jobs,clients,fabJobs,printJobs,stages,orderTypes}){
  ...
  const byType=orderTypes.map(t=>({type:t,count:filtered.filter(j=>j.orderType===t).length,rev:filtered.filter(j=>j.orderType===t).reduce((s,j)=>s+j.amount,0)})).filter(x=>x.count>0).sort((a,b)=>b.rev-a.rev);
...
// ── Settings ──────────────────────────────────────────────────────
function Settings({loyaltyGifts,setLoyaltyGifts,showToast,orderTypes,setOrderTypes,stages,setStages}){
  const[gifts,setGifts]=useState(()=>loyaltyGifts.map(g=>({...g})));
  const[types,setTypes]=useState(()=>[...orderTypes]);
  const[stgs,setStgs]=useState(()=>[...stages]);
  ...
  return (
    <div style={S.content}>
      ...
      <div style={S.card}>
        ...
        <div style={{fontSize:11,color:"#607080",marginBottom:10}}>These appear in the "Order Type" dropdown when creating jobs.</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:10}}>
          {types.map((t,i)=> (
            <div key={i} style={{display:"flex",alignItems:"center",gap:4,background:"#0d1520",border:"1px solid #1a2535",borderRadius:6,padding:"4px 8px"}}>
              <span style={{fontSize:11,color:"#c8d8e8"}}>{t}</span>
              <button onClick={()=>setTypes(prev=>prev.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",color:"#ff3b3b55",cursor:"pointer",fontSize:10,padding:"0 2px"}}>✕</button>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:6}}>
          <input style={{...S.input,flex:1}} value={newType} onChange={e=>setNewType(e.target.value)} placeholder="New order type…" onKeyDown={e=>{if(e.key==="Enter"&&newType.trim()){setTypes(prev=>[...prev,newType.trim()]);setNewType("");}}}/>
          <button onClick={()=>{if(newType.trim()){setTypes(prev=>[...prev,newType.trim()]);setNewType("");}}} style={{...S.submitBtn,padding:"6px 14px",fontSize:11}}>+ Add</button>
        </div>
        <div style={{display:"flex",gap:8,marginTop:10}}>
          <button onClick={async ()=>{setSaving(true);try{const{saveSettings}=await import("./supabase.js");await saveSettings("order_types",types);setOrderTypes(types);showToast("Order types saved!");}catch(e){showToast("Save failed","error");}setSaving(false);}} disabled={saving} style={{...S.submitBtn,fontSize:11,opacity:saving?0.6:1}}>💾 Save Types</button>
          <button onClick={()=>{setTypes([...DEFAULT_ORDER_TYPES]);showToast("Reset — click Save");}} style={{...S.submitBtn,background:"none",border:"1px solid #2a3545",color:"#607080",fontSize:11}}>↺ Reset</button>
        </div>
      </div>

      <div style={S.card}>
        <h3 style={S.cardTitle}>📋 Job Stages</h3>
        <div style={{fontSize:11,color:"#607080",marginBottom:10}}>These appear in the "Stage" dropdown. Order matters — first stage is the starting point, last is final.</div>
        <div style={{marginBottom:10}}>
          {stgs.map((s,i)=> (
            <div key={i} style={{display:"flex",alignItems:"center",gap:6,background:"#0d1520",border:"1px solid #1a2535",borderRadius:6,padding:"5px 10px",marginBottom:3}}>
              <span style={{fontSize:10,color:"#405060",fontWeight:700,minWidth:18}}>{i+1}.</span>
              <span style={{fontSize:11,color:STAGE_COLORS[s]||"#c8d8e8",flex:1,fontWeight:600}}>{s}</span>
              <button onClick={()=>{if(i>0)setStgs(prev=>{const n=[...prev];[n[i-1],n[i]]=[n[i],n[i-1]];return n;});}} style={{background:"none",border:"none",color:"#607080",cursor:"pointer",fontSize:10}}>▲</button>
              <button onClick={()=>{if(i<stgs.length-1)setStgs(prev=>{const n=[...prev];[n[i],n[i+1]]=[n[i+1],n[i]];return n;});}} style={{background:"none",border:"none",color:"#607080",cursor:"pointer",fontSize:10}}>▼</button>
              <button onClick={()=>setStgs(prev=>prev.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",color:"#ff3b3b55",cursor:"pointer",fontSize:10}}>✕</button>
            </div>
          ))}
        </div>
        <div style={{display:"flex",gap:6}}>
          <input style={{...S.input,flex:1}} value={newStage} onChange={e=>setNewStage(e.target.value)} placeholder="New stage name…" onKeyDown={e=>{if(e.key==="Enter"&&newStage.trim()){setStgs(prev=>[...prev,newStage.trim()]);setNewStage("");}}}/>
          <button onClick={()=>{if(newStage.trim()){setStgs(prev=>[...prev,newStage.trim()]);setNewStage("");}}} style={{...S.submitBtn,padding:"6px 14px",fontSize:11}}>+ Add</button>
        </div>
        <div style={{display:"flex",gap:8,marginTop:10}}>
          <button onClick={async ()=>{setSaving(true);try{const{saveSettings}=await import("./supabase.js");await saveSettings("stages",stgs);setStages(stgs);showToast("Stages saved!");}catch(e){showToast("Save failed","error");}setSaving(false);}} disabled={saving} style={{...S.submitBtn,fontSize:11,opacity:saving?0.6:1}}>💾 Save Stages</button>
          <button onClick={()=>{setStgs([...DEFAULT_STAGES]);showToast("Reset — click Save");}} style={{...S.submitBtn,background:"none",border:"1px solid #2a3545",color:"#607080",fontSize:11}}>↺ Reset</button>
        </div>
      </div>
    </div>
  );
