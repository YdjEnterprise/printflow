...
// ── Pending Jobs ───────────────────────────────────────────────────
function PendingJobs({jobs,setJobs,showToast,stages,deleteJob,orderTypes}){
      ...
      {[{label:"🔴 URGENT — ≤ 3 days",list:red,border:"#ff3b3b"},{label:"🟡 SOON — 4–5 days",list:yellow,border:"#f5a623"},{label:"🟢 ON TRACK — 6+ days",list:green,border:"#27c27c"}].map(sec=>sec.list.length>0&&(
        <div key={sec.label} style={{...S.card,borderLeft:`4px solid ${sec.border}`,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:9}}>
            {sec.list.map(j=><PCard key={j.orderId} job={j} border={sec.border} expanded={expandedId===j.orderId} setExpanded={setExpandedId} markDelivered={markDelivered} updateStage={updateStage} updatePaid={updatePaid} updateDeadline={updateDeadline} stages={stages} deleteJob={deleteJob}/>)}
          </div>
        </div>
      ))}
...
function PCard({job,border,expanded,setExpanded,markDelivered,updateStage,updatePaid,updateDeadline,stages,deleteJob}){
  const[confirmDel,setConfirmDel]=useState(false);
