...
function AddJob({jobs,setJobs,clients,setClients,fabJobs,setFabJobs,printJobs,setPrintJobs,showToast,setPage,orderTypes,stages}){
  ...
  const mkPrint=()=>({_id:Date.now()+Math.random(),type:"Digital Flex",material:"Star Flex",width:"",height:"",qty:"1",resolution:"720dpi",notes:""});
  const mkFab=()=>({_id:Date.now()+Math.random(),type:"MS Frame",pipeSize:"1\"",pipeThick:"2mm",woodType:"Plywood 18mm",acpThick:"4mm",width:"",height:"",notes:"",stage:"Pending"});
      ...
      <div style={S.formCard}>
        ...
        {form.needsPrint&&(
          <div style={{marginTop:12,padding:14,background:"#0d1a2e",borderRadius:9,border:"1px solid #4f8ef733"}}>
            ...
            {printSpecs.map((ps,idx)=> (
              <div key={ps._id} style={{background:"#0a1420",borderRadius:8,padding:12,marginBottom:8,border:"1px solid #1a2a3e",position:"relative"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontSize:10,color:"#4f8ef7",fontWeight:700}}>Print #{idx+1}{ps.width&&ps.height?" — "+ps.width+"×"+ps.height+"ft":""}</span>
                  {printSpecs.length>1&&<button onClick={()=>removePrint(idx)} style={{background:"none",border:"1px solid #ff3b3b33",color:"#ff3b3b",borderRadius:4,padding:"2px 7px",cursor:"pointer",fontSize:9}}>✕ Remove</button>}
                </div>
                <div style={S.formGrid}>
                  ...
                  <div style={S.fg}><label style={S.label}>Quantity</label><input type="number" style={S.input} value={ps.qty} onChange={e=>updatePrint(idx,"qty",e.target.value)} placeholder="1"/></div>
                  ...
                  <div style={{...S.fg,gridColumn:"1/-1"}}><label style={S.label}>Notes</label><input style={S.input} value={ps.notes} onChange={e=>updatePrint(idx,"notes",e.target.value)} placeholder="Lamination, finishing, etc."/></div>
                </div>
              </div>
            ))}
        ...
        {form.needsFabrication&&(
          <div style={{marginTop:12,padding:14,background:"#1a0d00",borderRadius:9,border:"1px solid #ff8c0033"}}>
            ...
            {fabSpecs.map((fs,idx)=> (
              <div key={fs._id} style={{background:"#120a00",borderRadius:8,padding:12,marginBottom:8,border:"1px solid #2a1a00",position:"relative"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontSize:10,color:"#ff8c00",fontWeight:700}}>Frame #{idx+1}{fs.width&&fs.height?" — "+fs.width+"×"+fs.height+"ft "+fs.type:""}</span>
                  {fabSpecs.length>1&&<button onClick={()=>removeFab(idx)} style={{background:"none",border:"1px solid #ff3b3b33",color:"#ff3b3b",borderRadius:4,padding:"2px 7px",cursor:"pointer",fontSize:9}}>✕ Remove</button>}
                </div>
                <div style={S.formGrid}>
                  ...
                  <div style={{...S.fg,gridColumn:"1/-1"}}><label style={S.label}>Notes</label><input style={S.input} value={fs.notes} onChange={e=>updateFab(idx,"notes",e.target.value)} placeholder="Finish, coating, extra details…"/></div>
                </div>
              </div>
            ))}
            ...
            <div style={{marginTop:10,padding:"9px 12px",background:"#0d1520",borderRadius:7,fontSize:11,color:"#607080"}}>
              <strong style={{color:"#ff8c00"}}>Auto-creates {fabSpecs.length} fabrication job{fabSpecs.length>1?"s":""}</strong> in the Fabrication Dept tab.
            </div>
          </div>
        )}
